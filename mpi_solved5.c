...
* Bug/Reason: The send buffer isn't completely full initially. So it keep flushing 
              data to send buffer and runs faster initially. But as send buffer is 
              actually full, it needs to wait until the data is actually recieved 
              by the second task. 
******************************************************************************/
#include "mpi.h"
#include <stdio.h>
